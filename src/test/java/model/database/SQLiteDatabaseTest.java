...
public class SQLiteDatabaseTest {
  ...
  public void isHistoryCreated() throws SQLException {
    ...
    res = state.executeQuery("SELECT name FROM sqlite_master WHERE type='table' AND name='history'");
    assertTrue(res.next());
    res.close();
    state.close();
    con.close();
  ...
  public void isRouteInsertIntoDatabase() throws SQLException {
    ...
    database.startCommite();
    res = state.executeQuery("select sourceAirport from 'routeInsertion' where sourceAirportID=2965");
  ...
  /**
   * Test that if airport table successfully initialised into database, if not exist then create one.
   */
  @Test
  public void isAirportTableInitialised() throws SQLException {
    database.initialiseTable("Airport", "airportTableinit.csv");
    state = con.createStatement();
    res = state.executeQuery("SELECT name FROM sqlite_master WHERE type='table' AND name='airportTableinit'");
    assertTrue(res.next());
    res.close();
    state.close();
    con.close();
  ...
  public void isRouteTableInitialised() throws SQLException {
    ...
    state = con.createStatement();
    res = state.executeQuery("SELECT name FROM sqlite_master WHERE type='table' AND name='routeTableinit'");
    assertTrue(res.next());
    res.close();
    state.close();
    con.close();
  ...
  /**
   * Test that if airline table successfully initialised into database, if not exist then create one.
   */
  @Test
  public void isAirlineTableInitialised() throws SQLException {
    database.initialiseTable("Airline", "airlineTableinit.csv");
    state = con.createStatement();
    res = state.executeQuery("SELECT name FROM sqlite_master WHERE type='table' AND name='airlineTableinit'");
    assertTrue(res.next());
    res.close();
    state.close();
    con.close();
  ...
  public void isFileDeleted() throws SQLException {
    ...
    state = con.createStatement();
    res = state.executeQuery("SELECT name FROM sqlite_master WHERE type='table' AND name='fileToBeDeleted.csv'");
    if(res.next()){
      database.deleteFile("fileToBeDeleted", "Airport");
      res = state.executeQuery("SELECT name FROM sqlite_master WHERE type='table' AND name='fileToBeDeleted.csv'");
