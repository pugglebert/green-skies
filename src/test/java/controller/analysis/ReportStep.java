...
public class ReportStep {
    ...
    private RouteAddToHistoryPopUpController routeAddToHistoryPopUpController = new RouteAddToHistoryPopUpController();
    ...
    @Given("The user has uploaded route data and airport data and added a route {string},{int},{string},{int},{string},{int},{string},{int},{string} and set the number of passengers to {int} into the flight history and is currently on the carbon emissions report page and the user has entered their goal as {int} kg")
    public void theAddedRouteToHistoryIs(String airline, int airlineID, String sourceAirport, int sourceAirportID,
                                         String destinationAirport, int destinationAirportID, String codeshare, int stops,
                                         String equipment, int timesTaken, int goal) throws FileNotFoundException, FileSystemException, SQLException {
        loader.loadFile("../seng202_project/src/test/java/TestFiles/airports.csv", "Airport");
        loader.loadFile("../seng202_project/src/test/java/TestFiles/routes.csv", "Route");
        Route historyRoute = new Route(airline, airlineID, sourceAirport, sourceAirportID, destinationAirport, destinationAirportID,
    ...
    public void theTotalDisplayedIs(double expectedEmissions) {
        Assert.assertEquals(expectedEmissions, this.generalStatsCalculator.getTotalCarbonEmissions(), 0.01);
